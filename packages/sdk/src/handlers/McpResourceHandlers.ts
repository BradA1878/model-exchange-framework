...
import { BaseEventPayload, createBaseEventPayload, McpResourceErrorEventPayload, McpResourceResultEventPayload } from '@mxf-dev/core/schemas/EventPayloadSchema';
import { McpResourceGetEventPayload, McpResourceEventData, McpResourceListEventPayload } from '@mxf-dev/core/schemas/EventPayloadSchema';
...
export class McpResourceHandlers extends McpHandler {
    ...
    private getResource = (id: string, channelId: string): Promise<any> => {
        return new Promise((resolve, reject) => {
            try {
                // Generate request ID
                const requestId = `resource-get-${uuidv4()}`;
    ...
    private listResources = (filter: string | undefined, channelId: string): Promise<any[]> => {
        return new Promise((resolve, reject) => {
            try {
                // Generate request ID
                const requestId = `resource-list-${uuidv4()}`;
