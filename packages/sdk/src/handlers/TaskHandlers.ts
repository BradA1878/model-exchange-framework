...
export class TaskHandlers extends Handler {
    ...
    private setupTaskRequestHandler(): void {
        const subscription = EventBus.client.on(Events.Task.REQUEST, async (payload) => {
            // Request fields travel in the envelope's data — reading them off
            // the envelope root silently matched nothing before.
            const task = payload.data as unknown as SimpleTaskRequest;
            try {
    ...
    private setupTaskResponseHandler(): void {
        const subscription = EventBus.client.on(Events.Task.RESPONSE, (payload) => {
            const response = payload.data as unknown as SimpleTaskResponse;
            try {
