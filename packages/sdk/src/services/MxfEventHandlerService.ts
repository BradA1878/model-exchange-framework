...
import { sharedStateService } from '@mxf-dev/core/services/SharedStateService';
import { detectOperation } from '@mxf-dev/core/schemas/MxpProtocolSchemas';
import { MxfStructuredPromptBuilder } from './MxfStructuredPromptBuilder.js';
    ...
    private compressContextForLLM(conversationHistory: ConversationMessage[]): ConversationMessage[] {
        ...
        return conversationHistory.map((msg, index) => {
            ...
            if (this.containsStateReference(content)) {
                ...
                const stateId = `ctx_${Date.now().toString(36)}_${uuidv4()}`;
