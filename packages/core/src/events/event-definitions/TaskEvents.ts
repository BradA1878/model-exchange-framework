...
import { ChannelTask, AgentAssignmentAnalysis, ChannelWorkloadAnalysis } from '../../types/TaskTypes.js';
 ...
 *
 * Re-exported from EventPayloadSchema so there is exactly ONE TaskEventPayload
 ...
 */
export type TaskCreatedEvent = TaskEventPayload & {
    data: {
        task: ChannelTask;
    };
};
 ...
 */
export type TaskAssignedEvent = TaskEventPayload & {
    data: {
        task: ChannelTask;
 ...
 */
export type TaskProgressEvent = TaskEventPayload & {
    data: {
        task: ChannelTask;
 ...
 */
export type AgentWorkloadEvent = TaskEventPayload & {
    data: {
...
export interface TaskPayloads {
    // Legacy events
    'task:request': TaskEventPayload;
    'task:response': TaskEventPayload;
    
    // Task request events
    'task:create_request': TaskEventPayload & { data: { task: Partial<ChannelTask> & Pick<ChannelTask, 'title'> } };
    'task:start_request': TaskEventPayload & { data: { task: ChannelTask } };
    'task:complete_request': TaskEventPayload;
    ...
    'task:progress_updated': TaskEventPayload;
    ...
    'task:workload_analyzed': TaskEventPayload;
