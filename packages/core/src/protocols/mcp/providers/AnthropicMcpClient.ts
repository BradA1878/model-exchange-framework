...
import { extractToolCalls, extractToolCallId, extractToolResultText } from '../utils/MessageConverters.js';
...
import { AnthropicMessageAdapter } from '../converters/adapters/AnthropicMessageAdapter.js';

const logger = new Logger('info', 'AnthropicMcpClient');
...
export class AnthropicMcpClient extends BaseMcpClient {
    ...
    protected async initializeProvider(): Promise<void> {
        ...
        logger.debug('Anthropic message adapter registered (client context)');
    }
    ...
    private structureMessagesFromContext(context: AgentContext): { systemPrompt: string; messages: AnthropicMessage[] } {
        ...
        // 2. Task message (if present) - Only inject if NOT already in conversation history
