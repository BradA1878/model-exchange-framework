...
import { TensorFlowEvents } from '../events/event-definitions/TensorFlowEvents.js';
import {
    SYSTEM_AGENT_ID,
    SYSTEM_CHANNEL_ID,
...
import { MxfMLService } from './MxfMLService.js';
import { MxfModelType } from '../types/TensorFlowTypes.js';
import { isTensorFlowEnabled, getTensorFlowConfig } from '../config/tensorflow.config.js';
...
export class PredictiveAnalyticsService extends EventEmitter {
    ...
    public async predictErrors(
        ...
        if (prediction.probability > 0.7) {
            EventBus.server.emit(Events.Analytics.HIGH_ERROR_RISK_PREDICTED, createBaseEventPayload(
                Events.Analytics.HIGH_ERROR_RISK_PREDICTED,
                agentId,
                channelId,
                {
                    ...
                    details: { importantFeatures: prediction.importantFeatures, source: prediction.source },
                    timestamp: new Date()
                }
            ));
        }
    ...
    public async detectAnomalies(
        ...
        for (const anomaly of anomalies) {
            ...
            if (anomaly.severity === 'critical' || anomaly.severity === 'high') {
                EventBus.server.emit(Events.Analytics.ANOMALY_DETECTED, createBaseEventPayload(
                    ...
                    channelId,
                    {
                        ...
                        metrics: { ...anomaly.context, score: anomaly.score, toolName },
                        timestamp: new Date()
                    }
                ));
            }
    ...
    private async generateTimingSuggestions(
        ...
        if (successByHour && successByHour[currentHour] < 0.7) {
    ...
    private updateHeuristicMetadata(sampleCount: number): void {
        ...
        if (modelMeta) {
            ...
            modelMeta.accuracy = null; // heuristics have no measured validation accuracy
        }
    }
    ...
    private getSystemContext(): { load: number; concurrentRequests: number } {
        // Real normalized CPU load. Concurrent request tracking has no data
        // source yet — 0 means "untracked", never a fabricated number.
        return {
    ...
    private async calculatePatternMatch(
        ...
        // No pattern store is wired up yet: return neutral evidence (0)
     ...
     */
    private async getSuccessRateByHour(_agentId: AgentId): Promise<number[] | null> {
        // No per-hour success metrics are recorded yet. Returning null keeps
        // the timing suggestion honest — fabricated rates produced fake
        // "your success rate is higher around X:00" advice.
        return null;
    }
    ...
    private async getPatternHistory(
        ...
        // Fail fast: there is no historical pattern store. Forecasting over
        // fabricated data produced confident-looking noise.
        throw new Error(
            `Pattern history tracking is not implemented — cannot forecast '${pattern}' without a data source`
        );
    }
