...
import { ValidationPerformanceService } from './ValidationPerformanceService.js';
import { PatternLearningService } from './PatternLearningService.js';
import { ProactiveValidationService } from './ProactiveValidationService.js';
import { AutoCorrectionService } from './AutoCorrectionService.js';
...
export class ValidationAnalyticsService extends EventEmitter {
    ...
    public startABTest(testId: string): void {
        ...
        EventBus.server.emit(Events.Analytics.AB_TEST_STARTED, createBaseEventPayload(
            Events.Analytics.AB_TEST_STARTED,
            'system',
            'global',
            {
                testId,
                testName: test.name,
                variants: test.variants.map(v => v.id),
                parameters: {},
                timestamp: new Date()
            }
        ));
