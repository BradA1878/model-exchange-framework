...
export class PhaseMemoryOperations {
    ...
    public async store(options: PhaseStorageOptions): Promise<PhaseStorageResult> {
        ...
        // Emit storage event
        EventBus.server.emit(OrparMemoryEvents.PHASE_MEMORY_STORED, createBaseEventPayload(OrparMemoryEvents.PHASE_MEMORY_STORED, options.agentId, options.channelId ?? 'unknown', {
            agentId: options.agentId,
    ...
    public recordUsage(
        ...
        EventBus.server.emit(OrparMemoryEvents.CYCLE_MEMORY_USAGE_RECORDED, createBaseEventPayload(OrparMemoryEvents.CYCLE_MEMORY_USAGE_RECORDED, agentId, channelId, {
            agentId,
            channelId,
            cycleId,
            phase,
