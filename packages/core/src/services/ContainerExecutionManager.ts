...
// The code-executor build context ships at <repo>/docker/code-executor and the
// server always runs from the repo root (bun dev, Docker WORKDIR). Resolved
// once from cwd — missing directory fails fast at the use sites.
const CODE_EXECUTOR_DIR = path.join(process.cwd(), 'docker', 'code-executor');
/**
...
export class ContainerExecutionManager {
    ...
    private async buildImage(): Promise<boolean> {
        try {
            const dockerfilePath = CODE_EXECUTOR_DIR;
            if (!fs.existsSync(path.join(dockerfilePath, 'Dockerfile'))) {
                this.logger.error(`Code executor build context not found at ${dockerfilePath} — expected docker/code-executor relative to the working directory`);
                return false;
            }
    ...
    public async execute(request: ContainerExecutionRequest): Promise<ContainerExecutionResult> {
        ...
        const containerId = `mxf-exec-${uuidv4()}`;
    ...
    private getSourceFilesModTime(): number {
        const possiblePaths = [CODE_EXECUTOR_DIR];
