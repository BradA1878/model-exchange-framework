...
import { ChannelMessage, AgentMessage } from './MessageSchemas.js';
...
} from '../events/event-definitions/DagEvents.js';
import {
    TensorFlowEvents,
...
} from '../events/event-definitions/TensorFlowEvents.js';
import {
    UserInputEvents,
...
} from '../events/event-definitions/UserInputEvents.js';
import {
    CompactionEvents,
...
export interface TaskResponseEventData {
    ...
    response: any;        // The task response content/details — see task field note
...
export interface TaskAssignmentEventData {
    ...
    task: any; // Full task object for consistency with other task events — see TaskEventData.task note
}
...
export interface ControlLoopSpecificData {
    ...
    config?: Record<string, unknown>;          // Control loop configuration
    observation?: Observation | null;     // Current observation
    reasoning?: Reasoning | null;       // Current reasoning
    plan?: Plan | null;            // Current plan
    action?: PlanAction | null;          // Current action (or action to be executed)
 ...
 */
export function createTaskEventPayload<TData extends TaskEventData>(
    eventType: EventName | string, 
    emittingAgentId: AgentId,
    channelId: ChannelId,
    taskData: TData,
    options: { source?: string; eventId?: string; timestamp?: number; } = {}
): TaskEventPayload & { data: TData } {
    ...
    if (task.assignmentScope) {
        ...
        if (task.assignmentScope === 'channel-wide') {
            validator.assert(
                task.channelWideTask === true || (task.maxParticipants ?? 0) > 0,
    ...
    // The spread widens the literal; the narrowed return is runtime-true
    // because data IS the TData object the caller passed (plus fromAgentId).
    return createBaseEventPayload(eventType, emittingAgentId, channelId, dataWithFromAgent as TData, options) as TaskEventPayload & { data: TData };
}
...
} from '../events/event-definitions/MxpEvents.js';
