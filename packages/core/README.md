...
Core foundation of the [Model Exchange Framework](https://github.com/mxf-dev/mxf) (MXF): the event system, payload schemas, shared types and interfaces, runtime configuration, MXP protocol layer, MCP protocol contracts, services, and utilities that the MXF server and the `@mxf-dev/sdk` agent client are built on.
