...
## Key Difference: LLM-Driven Tool Usage

Unlike demos that use direct `executeTool()` calls, this demo lets the agent **autonomously decide** which tools to use based on task requirements. The agent:
...
When you run the demo, watch for this autonomous behavior:
...
1. **Autonomous Tool Selection**: Agent decides when to use parameter tools
2. **Metacognitive Reasoning**: Agent explains why it needs upgrades
3. **Cost-Aware Behavior**: Agent resets after complex work
4. **Task-Based Adaptation**: Simple tasks = budget model, Complex = premium
...
3. Run the demo:
   ```bash
   bun run inference-params-demo.ts
   # Or use npm script:
...
The agent is configured with an `agentConfigPrompt` that instructs it on metacognitive behavior:
...
const agent = await sdk.createAgent({
  ...
  agentConfigPrompt: `You are a metacognitive agent...

    WORKFLOW:
    1. ASSESS COMPLEXITY - Simple vs Complex
    2. ADJUST PARAMETERS - Upgrade for complex tasks
    3. SOLVE THE PROBLEM
    4. RESET AFTER COMPLEX WORK - Save costs
...
### Old Approach (Direct Tool Calls)
```typescript
// Hardcoded - bypasses LLM reasoning
const result = await agent.executeTool('get_current_params', { phase: 'observation' });
await agent.executeTool('request_inference_params', { ... });
```

### New Approach (LLM-Driven)
```typescript
// Agent receives task and decides autonomously
await agent.mxfService.createTask({
  title: 'Solve Problems with Adaptive Parameters',
  description: '...problems of varying complexity...',
  assignedAgentIds: ['MetacognitiveAgent']
});
// Agent reasons: "Problem 2 is complex, I need to upgrade..."
// Agent calls tools based on its own assessment
```
