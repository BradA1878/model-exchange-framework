 ...
 * ============================================================================
 *
 * This example demonstrates the ORPAR cognitive cycle using a classic game:
 ...
 * 3. SystemLLM Integration  - Server-side orchestration of the cycle
 * 4. Event-Based Tracking   - Phase events broadcast for visualization
 * 5. Custom MCP Tools       - Game-specific tool server
 *
 * ORPAR IN ACTION:
 * ----------------
 * For the GUESSER (each question cycle):
 *   OBSERVE  → Look at question history and previous answers
 *   REASON   → Analyze patterns, narrow down possibilities
 *   PLAN     → Decide on strategy - what question to ask?
 *   ACT      → Execute: call game_askQuestion
 *   REFLECT  → Record outcome and evaluate what was learned
 *
 ...
 *
 *   OBSERVE  → game_getState, memory read tools, memory_search_*
 *   REASON   → orpar_reason only (pure analysis)
 *   PLAN     → planning_create, planning_view
 *   ACT      → Role-specific game tools (game_askQuestion, game_answerQuestion, etc.)
 *   REFLECT  → planning_update_item, memory write tools, task_complete
 *
...
async function connectAgents() {
    console.log('╔═══════════════════════════════════════════════════════════╗');
    console.log('║                                                           ║');
    console.log('║         TWENTY QUESTIONS: ORPAR COGNITIVE DEMO            ║');
    ...
    const roleAllowedTools = {
        thinker: ['game_getState', 'game_setSecret', 'game_answerQuestion', ...orparTools, ...planningTools, ...memoryTools],
        guesser: ['game_getState', 'game_askQuestion', 'game_makeGuess', ...orparTools, ...planningTools, ...memoryTools]
    };
    ...
    for (const [role, player] of Object.entries(PLAYERS)) {
        ...
        const agent = await sdk.createAgent({
            ...
            maxHistory: 10,    // CRITICAL: Keep history small to prevent slowdown! Each ORPAR cycle adds 5+ messages
            reasoning: {
            ...
            maxIterations: 15,  // Reduced from 20 - should be enough for ORPAR + game action
            agentConfigPrompt: player.personality,
            // Extended reconnection for long-running games
            reconnectAttempts: 20,  // More attempts for stability
            reconnectDelay: 2000    // Faster reconnection attempts
        });
    ...
    channel.on(Events.Orpar.OBSERVE, async (payload: any) => {
    ...
    channel.on(Events.Orpar.REASON, async (payload: any) => {
        const agentId = payload.agentId || payload.data?.agentId;
        const role = agentId === PLAYERS.thinker.id ? 'thinker' : 'guesser';
    ...
    channel.on(Events.Orpar.PLAN, async (payload: any) => {
        const agentId = payload.agentId || payload.data?.agentId;
        const role = agentId === PLAYERS.thinker.id ? 'thinker' : 'guesser';
    ...
    channel.on(Events.Orpar.ACT, async (payload: any) => {
        const agentId = payload.agentId || payload.data?.agentId;
        const role = agentId === PLAYERS.thinker.id ? 'thinker' : 'guesser';
    ...
    channel.on(Events.Orpar.REFLECT, (payload: any) => {
        const agentId = payload.agentId || payload.data?.agentId;
        const role = agentId === PLAYERS.thinker.id ? 'thinker' : 'guesser';
    ...
    const cleanup = async () => {
        ...
        try {
            ...
            await sdk.unregisterChannelMcpServer(channelId, 'twenty-questions-mcp-server').catch(() => {});
            ...
            // Delete agent memories via API (must be done before channel deletion)
            console.log('Deleting agent memories via API...');
            for (const player of Object.values(PLAYERS)) {
                if (cleanupCredentials) {
                    await fetch(`${mxfServerUrl}/api/agents/${player.id}/memory`, {
                        method: 'DELETE',
                        headers: {
            ...
            console.log('Deleting channel via API...');
            if (cleanupCredentials) {
...
Categories: animal, object, food, place, vehicle, plant, or any fitting category.`,
    ...
    const playGame = async () => {
        ...
        while (true) {
            ...
            // Brief delay to let cancellation propagate before clearing history
...
Tools are automatically updated for each phase.`;
            ...
            // Brief delay between turns for clean separation (reduced from 500ms)
