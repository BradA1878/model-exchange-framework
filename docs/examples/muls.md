...
The Memory Utility Learning System treats memory retrieval as a **decision problem** rather than pure similarity search. Memories are scored using both semantic similarity and Q-values that track how useful each memory has been for task completion.
...
1. **Q-values track memory utility** over time based on task outcomes
2. **Composite scoring** balances similarity with proven usefulness
3. **Phase-specific lambda** optimizes retrieval for different cognitive needs
4. **Manual reward injection** allows explicit memory feedback
...
Restart the server - the tool registration order ensures MULS tools are registered before McpService initializes.
...
See the full implementation in `examples/muls-demo/`

## Related Documentation

- [Memory Utility Learning System (MULS)](../mxf/memory-utility-learning.md)
